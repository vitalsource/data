...
def get_assess_ecm_glossary_comparison_answer_prompt( question, answer, feedback ):
...
def assess_ecm_glossary_comparison_answer( prompt, verbose=False ):
    ...
    correct = { 'true': True, 'false': False }[ re.search( r"<correct>(.*?)</", completion ).group( 1 ).lower() ]
...
def assess_ecm_answers( question_data ):
    ...
    for i_row, row in enumerate( question_data.itertuples(), 1 ):
        ...
        prompt = get_assess_ecm_glossary_comparison_answer_prompt( question, answer, feedback )
        try:
            genuine, correct = assess_ecm_glossary_comparison_answer(
                prompt, verbose=True
            )
