...
TODO***CHECK***When a passage is simplified, the original and simplified texts are
shown sequentially in an interactive panel. Screenshots of the
simplifier interface are shown below:
...
To load the events data into a pandas dataframe:
...
| `vbid`                              | Anonymized textbook identifier                             |
...
| `lex_sel_chars_per_word`            | Average word length (original passage)                     |
| `lex_sim_chars_per_word`            | Average word length (simplified passage)                   |
...
| `syn_sel_words_per_sentence`        | Average sentence length in words (original passage)        |
| `syn_sim_words_per_sentence`        | Average sentence length in words (simplified passage)      |
| `sem_sel_words`                     | Word count (original passage)                              |
| `sem_sim_words`                     | Word count (simplified passage)                            |
| `rd_fkgl_difference`                | FKGL change (simplified - original)                        |
| `rd_fre_difference`                 | FRE change (simplified - original)                         |
| `lex_content_log_p_difference`      | Word frequency change (simplified - original)              |
| `lex_chars_per_word_difference`     | Word length change (simplified - original)                 |
| `syn_dep_depth_difference`          | Dependency depth change (simplified - original)            |
| `syn_words_per_sentence_difference` | Sentence length change (simplified - original)             |
| `sem_cosine_similarity`             | Cosine similarity between original and simplified passage  |
| `sem_compression_ratio`             | Ratio of simplified/original word count                    |
